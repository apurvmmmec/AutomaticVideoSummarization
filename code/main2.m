...
rootDir = 'test1/';
timelapseVideoName = 'timelapse1.avi';
...
frames = load_sequence_color(datasetPath,'',101,numFrames+100,4,'jpg');
% frames = load_sequence_color(datasetPath,'',1,1200,4,'jpg');
...
% Main Loop
% We read each frame and then compare it with previous frame
...
for i=1:numOutput
    % Modify the resised progressbar to add indicator of current frame on progressbar 
    modPb = pbResized(:,:,:);
    
    currFrame = outFrames(i,1);
    
    seekBarLoc = ceil((currFrame/numFrames)*cols);
...
plot(dArr);
title('Image Intensity Variation between consecutive frames in a video');
xlabel('Frames');
ylabel('Difference between 2 consecutive frames')
disp('done');